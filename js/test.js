...
}; /** 通过传入坐标点数组方式飞行	* 参数 Array positions点数组，Object Model 模型对象,Number time飞行时间 以毫秒为单位	* 执行飞行函数	**/
flyRouter.prototype.flyRouterByPositions = function(positions = [], model, time) {
	if(positions.length == 0) {
		return;
	} /*var positionA = az.geometry.coordinates;	    for (i = 0; i < positionA.length; i++) {	        var x = positionA[i][0];	        var y = positionA[i][1];	        positions.push({ x: x, y: y });	    }*/
	...
	var stop = Cesium.JulianDate.addSeconds(start, time, new Cesium.JulianDate()); //Make sure viewer is at the desired time.	    
	...
	this.flyEntity = this.viewer.entities.add({ //Set the entity availability to the same interval as the simulation time.	        
		...
		model: model, //Show the path as a pink line sampled in 1 second increments.	        
		path: {
			resolution: 1,
...
flyRouter.prototype.stopFly = function() {
	...
	var stop = Cesium.JulianDate.addSeconds(start, 300000000, new Cesium.JulianDate());	    viewer.clock.stopTime = stop.clone();*/
	this.viewer.clock.multiplier = 0;
	if(this.flyEntity) {
